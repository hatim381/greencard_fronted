...
import IAFormPredictGoodSale from './IAFormPredictGoodSale';
...
const Dashboard = ({ user, setUser }) => {
  ...
  // Ajout des hooks pour le formulaire de virement
  const [showWithdrawalForm, setShowWithdrawalForm] = useState(false);
  const [withdrawalAmount, setWithdrawalAmount] = useState('');
  const [withdrawalIban, setWithdrawalIban] = useState('');
  const [withdrawalBic, setWithdrawalBic] = useState('');
  ...
  const handleWithdrawalSubmit = async (e) => {
    ...
    try {
      ...
      setWithdrawalMessage('Demande envoyée !');
      setShowWithdrawalForm(false);
      setWithdrawalAmount('');
      setWithdrawalIban('');
      setWithdrawalBic('');
      setWithdrawalRib('');
      // Met à jour le solde du portefeuille dans le state utilisateur
      if (res.data && typeof res.data.new_wallet_balance !== 'undefined' && setUser) {
        setUser({ ...user, wallet_balance: res.data.new_wallet_balance });
      }
    } catch (err) {
      setWithdrawalMessage("Erreur lors de l'envoi");
    }
  };
        ...
        <div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              {showWithdrawalForm && (
                <div style={{ background: '#fff', border: '1px solid #22C55E', borderRadius: 8, padding: 16, marginTop: 12 }}>
                  <h4>Demande de virement</h4>
                  <form onSubmit={handleWithdrawalSubmit}>
                    <div style={{ marginBottom: 8 }}>
                      <label>Montant (€) : </label>
                      <input type="number" step="0.01" min="1" required value={withdrawalAmount} onChange={e => setWithdrawalAmount(e.target.value)} />
                    </div>
                    <div style={{ marginBottom: 8 }}>
                      <label>IBAN : </label>
                      <input type="text" required value={withdrawalIban} onChange={e => setWithdrawalIban(e.target.value)} />
                    </div>
                    <div style={{ marginBottom: 8 }}>
                      <label>BIC : </label>
                      <input type="text" value={withdrawalBic} onChange={e => setWithdrawalBic(e.target.value)} />
                    </div>
                    <div style={{ marginBottom: 8 }}>
                      <label>RIB : </label>
                      <input type="text" value={withdrawalRib} onChange={e => setWithdrawalRib(e.target.value)} />
                    </div>
                    <button type="submit" className="btn btn-success">Envoyer</button>
                    <button type="button" className="btn btn-secondary" style={{ marginLeft: 8 }} onClick={() => setShowWithdrawalForm(false)}>Annuler</button>
                  </form>
          ...
          <div style={{
            ...
            <IAFormPredictGoodSale />
            <hr style={{ margin: '2em 0', border: 0, borderTop: '1.5px solid #E5E7EB' }} />
            {/* Meilleur produit */}
            <IAFormMeilleurProduit />
          </div>
